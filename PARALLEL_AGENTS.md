...
### 5.2 M5 fixture root (M3-emitted captured-transport `.npz` set)

M3 emits per-chgroup transport-TX captures from the chunks-5/6 voltage-
fixture path so M5 can wire its `bench/voltage_fixture_search.py`
operator-gate against real (not synthetic) input. Canonical layout:
...
Active fixture roots (regenerated by `bench/m3_emit_m5_fixtures.py`):

- `/home/ubuntu/data/m5_fixtures/0319/` — 15 chgroups (sb12 absent in
  the 0319 dump), `t_int_fast_native=4096` (~134 ms cadence, 1 fast-vis
  tile per fada block, 15 tiles total per chgroup), 3.4 MiB total.
  Continuum source for static-sky / continuum-imager checks. Phase
  centre = OVRO latitude (zenith) since 0319 is fed at zenith.
- `/home/ubuntu/data/m5_fixtures/250924mptq/` — 16 chgroups,
  `t_int_fast_native=64` (= 2097.152 µs cadence; 512 fast-vis tiles /
  chgroup ≈ 1.07 s per chgroup), 109 MiB total. Phase centre = burst
  declination (53.849°) so the F21 cal-DEC fold steers the source to
  `(l, m) ≈ (0, 0)`. Dispersion sweep is fully resolved across the band
  per-chgroup (cf. chunk-6 inter-chgroup top delays 0..6464 native
  samples).

**Per-chgroup `.npz` schema** (33 fields; the F26 sparse-COO transport
contract plus the SparsityPattern fields plus truth metadata):

| field | dtype / shape | provenance |
|---|---|---|
| `vis_cube_sparse` | `complex64 (N_DM=1, n_fv_total, N_filled)` | post-grid sparse-COO; the F26 transport-TX value channel |
| `ix_row, ix_col` | `uint16 (N_filled,)` | SparsityPattern UV table |
| `pattern_id` | `uint64` | M5 verifies locally via `predict_pattern_id` |
...
| `run_id, cal_path, voltage_path, git_sha, utc_iso` | strings | provenance |

`manifest.json` carries the per-chgroup record list + run-wide metadata.

**Owner**: M3. **Consumer**: M5 (`voltage_fixture_search.py`). Re-emit
when chunk 9's F25 multi-DM-trial path lands at the chunk-6 bench (then
`vis_cube_sparse` will have `N_DM > 1`); the schema is forward-compatible
because the leading DM axis is always present.
