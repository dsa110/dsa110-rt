...
| F8 | §3.1 line ~418 | Document the **mixed edge/centre band-edge convention** explicitly: ν_top_proc = 1.49875 GHz uses the **upper edge** of ch 1024 (matches §3.1 line 408 chgroup table), but ν_bot_proc = 1.311265 GHz uses the **centre** of ch 7167 (= `f0 - 7167.5·Δν`). The §3.1 closed-form `freq(g, ch) = f0 - (ch0+ch)·Δν` returns the upper edge → `freq(15, 383) = 1.311280 GHz`, NOT the pinned bot. The half-channel mismatch (15 kHz) is operationally absorbed by §3.6.2 stage-2 `numpy.rint(Δτ / t_int_fast_us)` (rounds to 0 at all DMs ≤ 3000), which is what makes `time_shift_corr_stage2[15, c] == 0` work. Add a §3.1 sub-paragraph stating the convention is intentional and the rounding is the operational reconciliation. | Agent C |
| F9 | §3.1 line ~418 | Add a derivation showing `BW_proc = ν_top_proc - ν_bot_proc = 6143.5·Δν = 187.485 MHz` (from the F8 mixed convention), **NOT** `N_chan_proc · Δν = 6144·Δν = 187.500 MHz` (which would be edge-edge, half a channel wider). The 15-kHz difference matters because it propagates into intra-block dispersion delay and the search-side residual table. | Agent C |
