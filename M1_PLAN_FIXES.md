...
| F12 | §3.6.3 line 791 | Plan claims residual intra-chgroup smearing at max δdm is `< t_int_fast_us / 30 ≈ 9 µs ≈ 0.017 t_int_search_us samples`, but this only holds under an assumed ~30-fine-per-coarse density. Our `gen_dmtrials_step` recursion (faithful port of legacy) produces ~43 fine/coarse on average over [0, 3000] but the high-DM tail has wider coarse cells (the step grows ~DM) so smearing at the max δdm in those cells is `O(34 ms)`, three orders of magnitude above the 9 µs claim. Update plan-prose to (a) state the smearing bound at MEAN δdm, not max; (b) note the SNR-loss-at-max-δdm depends on operating point and is benched in M5; (c) replace the 9-µs literal with a per-ops-row table populated by `bench/dm_plan_smearing.py` (deferred to M5+). M1 `test_dm_plan_time_shift_tables` asserts the soft mean-bound `mean_smearing < t_int_search_us` instead. | M1 author build-test |
